    ...
    def start(self):
        ...
        while(not game.is_over()):
            while(side == game.whose_turn()):
                ...
                print("What do you want to move? For example say: 'Move 5A to 4B'")
                                                                #TODO say a random move
                move = input("")
                #TODO prolog checking the string to see if the person is talking correctly
                moves = move.split()
                possible_moves = game.get_possible_moves()
                if("bye" in moves):
                    return
                while(True):
                    while(not self.checkPiece(moves[0]) or not self.checkPiece(moves[1])):
                        print(moves[0], "is not a valid move.")
                        print(moves[1], "is not a valid move.")
                        #TODO bot will say why its a invalid move
                        print("No no no no for example say: 'Move 5A to 4B'")
                                                        #TODO say a random move
                        move = input("")
                        #TODO prolog checking the string to see if the person is talking correctly
                        moves = move.split()
